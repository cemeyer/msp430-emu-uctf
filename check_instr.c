...
START_TEST(test_peephole12)
{
	...
	ck_assert(sexp_eq(exp, res));
}
END_TEST

START_TEST(test_peephole13)
{
	struct sexp *test =
	    mksexp(S_AND, 2,
		mksexp(S_RSHIFT, 2,
	...
	ck_assert(sexp_eq(sexp_imm_alloc(0), res));
}
END_TEST

START_TEST(test_peephole14)
{
	struct sexp *test =
	    mksexp(S_RRA, 2,
		mksexp(S_RRA, 2,
...
suite_instr(void)
{
	...
	tcase_add_test(tjmp, test_jmp_n);
	tcase_add_test(tjmp, test_jmp_n2);
